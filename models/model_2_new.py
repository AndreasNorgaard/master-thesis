...
class Model2:
    ...
    def _profit_expr(self, model):
        """Total profit (DKK) over the horizon as a Pyomo expression."""
        return sum(
            self.delta_t
            * (
                model.da_sell[q] * (model.da_price[q] - model.tariff_prod)
    ...
    def _co2_expr(self, model):
        """Total CO2 emissions (kg) over the horizon as a Pyomo expression."""
        return sum(
            self.delta_t * model.gamma[q] * (model.da_buy[q] - model.da_sell[q])
            for q in model.quarters
        )
    ...
    def _build_model(self):
        """Build the Pyomo model (parameters, variables, constraints) without an objective."""
    ...
    def _lex_solve(self, priority: str):
        """
        Lexicographic optimization in the (profit, co2) objective space.

        priority='profit': first maximize profit, then minimize co2 → y^UL
        priority='co2':    first minimize co2,    then maximize profit → y^LR
        """
        ...
        if priority == "profit":
            model.obj = pyo.Objective(expr=self._profit_expr(model), sense=pyo.maximize)
            solver.solve(model)
            best = pyo.value(model.obj)
            model.del_component("obj")
            model.lex_constr = pyo.Constraint(
                expr=self._profit_expr(model) >= best - eps
            )
            model.obj = pyo.Objective(expr=self._co2_expr(model), sense=pyo.minimize)
            solver.solve(model)
        elif priority == "co2":
            model.obj = pyo.Objective(expr=self._co2_expr(model), sense=pyo.minimize)
            solver.solve(model)
            best = pyo.value(model.obj)
            model.del_component("obj")
            model.lex_constr = pyo.Constraint(expr=self._co2_expr(model) <= best + eps)
            model.obj = pyo.Objective(expr=self._profit_expr(model), sense=pyo.maximize)
            solver.solve(model)
        else:
            raise ValueError(f"Unknown priority: {priority}")

        profit = pyo.value(self._profit_expr(model))
        co2 = pyo.value(self._co2_expr(model))
        return model, profit, co2

    def _weighted_solve(self, lam: float):
        """
        Weighted-sum scalarization used in Phase One (line 9).
        In min form: min λ·(-profit) + co2  ≡  max λ·profit − co2.
        """
        solver = pyo.SolverFactory("glpk")
        model = self._build_model()
        model.obj = pyo.Objective(
            expr=lam * self._profit_expr(model) - self._co2_expr(model),
            sense=pyo.maximize,
        )
        solver.solve(model)
        profit = pyo.value(self._profit_expr(model))
    ...
    def visualize_profit_distribution(self, breakdown: dict) -> None:
        """Plot a waterfall chart of the profit breakdown at the max-profit extreme y^UL."""
        labels = [
            "Day-ahead Revenue",
            "Production Tariffs",
            "Day-ahead Cost",
        ...
        fig.update_layout(
            title="Profit Distribution - Model 2 (max-profit extreme y^UL)",
            yaxis_title="DKK",
            plot_bgcolor="aliceblue",
            showlegend=False,
        )
        fig.show()
        fig.write_image("results/model_2_profit_distribution.png")

    def pareto_frontier(self, tol: float = 1e-6) -> list[dict]:
        """
        Phase One – Find supported extreme nondominated points (Figure 3.4).

        Biobjective space: f1 = -profit (min), f2 = co2 (min), so the algorithm's
        (y_1, y_2) maps to (-profit, co2). Equivalently we work directly with
        (profit, co2) and adapt the comparisons.
        """
        print("\nComputing lexicographic extremes...")
        model_UL, p_UL, c_UL = self._lex_solve("profit")  # y^UL: lex max profit
        _, p_LR, c_LR = self._lex_solve("co2")  # y^LR: lex min co2
        print(f"  y^UL = (profit={p_UL:.2f}, co2={c_UL:.2f})")
        print(f"  y^LR = (profit={p_LR:.2f}, co2={c_LR:.2f})")

        # Line 4: stop if both lexicographic extremes coincide
        if abs(p_UL - p_LR) < tol and abs(c_UL - c_LR) < tol:
            print("Only one nondominated point.")
            _, _, breakdown = self._extract_objectives(model_UL)
            self.visualize_profit_distribution(breakdown)
            return [{"profit": p_UL, "co2": c_UL}]

        # Line 5–6: initialise ordered list Y_N with pointer i for y+ = Y_N[i]
        Y_N: list[tuple[float, float]] = [(p_UL, c_UL), (p_LR, c_LR)]
        i = 0

        # Line 7–13: dichotomic search for supported extreme points
        while not (abs(Y_N[i][0] - p_LR) < tol and abs(Y_N[i][1] - c_LR) < tol):
            p_plus, c_plus = Y_N[i]
            p_minus, c_minus = Y_N[i + 1]

            # Line 8: λ = (y_2^+ - y_2^-)/(y_1^- - y_1^+) with y_1=-profit, y_2=co2
            #          = (c_plus - c_minus)/(p_plus - p_minus)
            lam = (c_plus - c_minus) / (p_plus - p_minus)

            # Line 9: solve weighted-sum subproblem max(λ·profit − co2)
            _, p_star, c_star = self._weighted_solve(lam)
            f_lam_star = -lam * p_star + c_star
            f_lam_plus = -lam * p_plus + c_plus

            print(
                f"  iter i={i}: λ={lam:.4g}, "
                f"y*=(profit={p_star:.2f}, co2={c_star:.2f}), "
                f"f_λ(x*)={f_lam_star:.4f} vs f_λ(y+)={f_lam_plus:.4f}"
            )

            # Line 10: strictly better → new supported extreme between y+ and y-
            if f_lam_star < f_lam_plus - tol:
                Y_N.insert(i + 1, (p_star, c_star))
            else:
                # Line 11–12: y+ := y-; y- := Next(Y_N, y+)
                i += 1

        print(f"Found {len(Y_N)} supported extreme nondominated points.")

        # Print breakdown and waterfall for the max-profit extreme y^UL
        _, _, breakdown = self._extract_objectives(model_UL)
        self.visualize_profit_distribution(breakdown)

        return [{"profit": p, "co2": c} for p, c in Y_N]
    ...
    def visualize_pareto_frontier(self, results: list[dict]):
        ...
        labels = [f"y^{k + 1}" for k in range(len(results))]
